...
-- retailer overrides change local presentation without mutating canonical copy.
...
create type public.knowledge_education_topic as enum (
  'mills',
  'fibres',
  'fabrics',
  'weaves',
...
create type public.knowledge_relation_kind as enum (
  'related',
  'prerequisite',
  'comparison',
  'follow_up'
...
create table public.knowledge_objects (
  ...
  image_url text,
  display_types public.knowledge_display_type[] not null check (
    cardinality(display_types) between 1 and 5
  ),
  commercial_intent public.knowledge_commercial_intent not null,
  education_topic public.knowledge_education_topic not null,
...
create index knowledge_objects_canonical_active_topic_idx
  on public.knowledge_objects (education_topic, active, priority desc)
  where retailer_id is null and deleted_at is null;

create index knowledge_objects_retailer_active_topic_idx
  on public.knowledge_objects (retailer_id, education_topic, active, priority desc)
  where retailer_id is not null and deleted_at is null;
...
create index knowledge_object_concepts_concept_idx
  on public.knowledge_object_concepts (concept_id, knowledge_object_id)
  where deleted_at is null;
...
begin
  ...
  select object.retailer_id into v_object_retailer_id
  from public.knowledge_objects as object
  where object.id = new.knowledge_object_id
  ...
  if v_object_retailer_id is null then
    if v_concept_retailer_id is not null then
      raise exception 'Canonical knowledge may link canonical concepts only';
    end if;
  elsif (
    v_concept_retailer_id is not null
    and v_concept_retailer_id <> v_object_retailer_id
  ) then
    raise exception 'Knowledge concept does not belong to the retailer';
  end if;
...
create table public.knowledge_object_relations (
  id uuid primary key default gen_random_uuid(),
  retailer_id uuid references public.retailers (id) on delete cascade,
  source_knowledge_object_id uuid not null references public.knowledge_objects (id) on delete restrict,
  target_knowledge_object_id uuid not null references public.knowledge_objects (id) on delete restrict,
  kind public.knowledge_relation_kind not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  deleted_at timestamptz,
...
create unique index knowledge_object_relations_canonical_uidx
...
create unique index knowledge_object_relations_retailer_uidx
  on public.knowledge_object_relations (
    retailer_id,
    source_knowledge_object_id,
    target_knowledge_object_id,
...
begin
  ...
  select object.retailer_id into v_source_retailer_id
  from public.knowledge_objects as object
  where object.id = new.source_knowledge_object_id
    and object.deleted_at is null;
  if not found then
    raise exception 'Source knowledge object is unavailable';
  end if;

  select object.retailer_id into v_target_retailer_id
  from public.knowledge_objects as object
  where object.id = new.target_knowledge_object_id
    and object.deleted_at is null;
  if not found then
  ...
  if new.retailer_id is null then
    if v_source_retailer_id is not null or v_target_retailer_id is not null then
      raise exception 'Canonical relations may reference canonical knowledge only';
    end if;
  elsif (
    ...
    raise exception 'Knowledge relation objects do not belong to the retailer';
  end if;
...
create table public.retailer_knowledge_overrides (
  ...
  title_override text check (
    ...
    or length(btrim(title_override)) between 1 and 200
  ),
...
create trigger enforce_retailer_knowledge_override_tenancy_on_write
  ...
  for each row execute function public.enforce_retailer_knowledge_override_tenancy();
...
create policy "authorized staff can read visible knowledge object concepts"
  ...
  using (
    ...
    or (
      exists (
        select 1
        from public.knowledge_objects as object
        where object.id = knowledge_object_id
          and object.deleted_at is null
          and (
            object.retailer_id is null
            or object.retailer_id = (select public.current_retailer_id())
          )
      )
      and (select public.current_retailer_role()) in (
        'read_only',
  ...
  with check (
    ...
    or (
      exists (
        ...
        where object.id = knowledge_object_id
          and object.deleted_at is null
          and object.retailer_id = (select public.current_retailer_id())
      )
      and (select public.current_retailer_role()) in ('manager', 'admin', 'owner')
    )
  );
...
create policy "authorized staff can read visible knowledge object relations"
...
comment on table public.knowledge_object_concepts is
  'Active concept links that gate knowledge eligibility through accepted metadata.';
comment on table public.knowledge_object_relations is
  'Canonical or retailer-owned typed relationships between knowledge objects.';
comment on table public.retailer_knowledge_overrides is
  'Tenant presentation, priority, hide, and pin controls that never mutate canonical copy.';
