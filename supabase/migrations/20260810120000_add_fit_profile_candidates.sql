...
-- reviews it against the previous approved fit and either approves or
-- rejects. Each candidate tracks which observations contributed to it and
-- records the advisor's decision. Approval does not write a customer-level
-- measurement row: migration 20260719000101's founder clarification
-- (superseding ADR-015) states PAON "does not own generic customer
-- measurements," and renamed that table to
-- legacy_customer_fit_profile_entries with all access revoked. In the
-- garment-first model, this candidate's own row plus its linked
-- fitting_observations are the authoritative record of the approved
-- revision.
...
-- identity and validates observations belong to the same garment/session
-- and retailer. customer_id is derived from the observations' own
-- fitting_session (fitting_observations has no customer_id column;
-- physical_garment_id is a garment, not the customer).
...
begin
  ...
  select * into v_session from public.fitting_sessions
    where id = v_first_observation.fitting_session_id;
  if not found then
    raise exception 'Fitting session not found for observation';
  end if;
  ...
  insert into public.fit_profile_candidates (
    ...
    v_first_observation.retailer_id, v_session.customer_id,
...
-- Advisor approves a candidate, advancing it to 'advisor_approved' status.
-- Does NOT write a customer-level measurement row: the founder
-- clarification in migration 20260719000101 (superseding ADR-015)
-- states PAON "does not own generic customer measurements" — the old
-- customer_fit_profile_entries table that concept fed was renamed to
-- legacy_customer_fit_profile_entries and had all access revoked at that
-- same migration. In the garment-first model this candidate's own row
-- (proposed_measurements) plus its linked fitting_observations already
-- are the authoritative record of the approved fit revision.
