-- FT-09: complete the "shared-look -> appointment/proposal outcome" gap.
-- book_appointment_from_consultation (20260806000000) already validates that
-- an optional p_message_attachment_id belongs to the conversation, but the
-- validated id was never persisted or returned to the caller -- the outcome
-- was silently discarded. This records it so a "shared look" (a
-- wardrobe-item-tagged or Pinterest attachment the customer sent in-thread)
-- is provably the origin of the appointment it produced.

alter table public.appointments
  add column if not exists origin_message_attachment_id uuid
    references public.message_attachments (id) on delete set null;

create index if not exists appointments_origin_attachment_idx
  on public.appointments (origin_message_attachment_id)
  where origin_message_attachment_id is not null;
...
begin
  ...
  -- Validate optional attachment (if provided, it must belong to this
  -- conversation) and hold it in v_attachment so it is actually persisted
  -- below -- previously validated then discarded.
  ...
  -- Create the appointment with origin_message_thread_id and, when
  -- provided, origin_message_attachment_id set.
