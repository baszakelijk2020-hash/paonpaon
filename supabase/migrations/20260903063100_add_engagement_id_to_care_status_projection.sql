-- Extend get_my_service_care_status() to include engagement_id and partner_id for quality review wiring.
-- PHASE 12.3: customer quality review form needs engagement ID and partner ID to submit reviews.

drop function if exists public.get_my_service_care_status();

create function public.get_my_service_care_status()
returns table (
  booking_id uuid,
  engagement_id uuid,
  partner_id uuid,
...
comment on function public.get_my_service_care_status() is
  'FT-14 customer-safe HighMaintenance projection. Derives the caller from auth.uid(); never exposes partner, instructions, condition notes, cost or staff data. Includes engagement_id/partner_id/retailer_id so a customer can submit a quality review without a client-side lookup against service_partner_engagements, which RLS reserves for retailer staff.';
