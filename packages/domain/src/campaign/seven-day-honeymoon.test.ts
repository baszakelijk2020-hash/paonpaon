...
describe("seven-day and honeymoon packages", () => {
  ...
  it("derives honeymoon actions with stock, lead-time and pressure suppression", () => {
    ...
    expect(suppressed.find((action) => action.kind === "preparation")?.suppressed).toBe(
      true,
    );
    expect(
    ...
    expect(ready.find((action) => action.kind === "aftercare")?.suppressed).toBe(
      false,
    );
    ...
    expect(
      pressure.every((action) => action.suppressed),
    ).toBe(true);
  });
});
