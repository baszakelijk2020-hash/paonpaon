...
export function composeSevenDayOwnedFirstPlan(args: {
  ...
  const suggestedBySlot = new Map<OutfitSlotKind, SuggestedCatalogueCandidate[]>();
  ...
  for (let day = 1; day <= CAMPAIGN_CHALLENGE_DAY_COUNT; day += 1) {
    for (const slotKind of SEVEN_DAY_SLOT_KINDS) {
      ...
      const suggested = suggestedList[(day - 1) % Math.max(suggestedList.length, 1)];
...
export function separateOwnedAndSuggested(
  slots: readonly SevenDaySlotPlan[],
): {
