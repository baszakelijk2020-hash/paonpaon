...
Automatic frontier context rollover (AGENTS.md ch.55): this conversation's context is disposable — repository state is authoritative. Watch your own context growth across this turn. At roughly 150k-200k active context, start considering a rollover; by roughly 250k, end this turn at the next safe coherent boundary unless a bounded Route-C decision is actively in progress and stopping now would lose necessary unresolved reasoning. A safe boundary requires: no unresolved Git merge/rebase/cherry-pick; no uncommitted implementation that would be hard to reconstruct; no running worker whose result still needs interpreting from this conversation alone; current material capability/blocker state recorded in PHASE.md/PROJECT_STATE.md; accepted work committed. When you reach a safe boundary at large context, finish the current bounded slice, verify it, commit and push, update PHASE/PROJECT_STATE only where materially necessary, then simply end this turn normally with PAON_RUNNER_STATUS: CONTINUE below — do not write a conversation summary or handoff file, and do not ask the founder anything first. The outer runner always cold-starts the next turn from repository state, never from this transcript, so ending the turn here is CONTINUE, not STOP. Do not let an ordinary turn drift toward 500k+ context merely because you still can continue.
