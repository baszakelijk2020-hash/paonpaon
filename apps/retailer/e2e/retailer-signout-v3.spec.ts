...
/**
 * Retailer sign-out control (GLOBAL scope, founder decision 2026-09-01).
 *
 * The dashboard layout mounts the `signOut` server action
 * (apps/retailer/app/(dashboard)/actions.ts) as two SEPARATE
 * `SignOutButton` instances — one in the desktop sidebar, one in the mobile
 * drawer — rather than passing one server-rendered `<form>` element into
 * both AppShell slots (docs/PHASE.md "REAL BUG FOUND, NOT FIXED
 * 2026-08-19": a shared element instance produced a click that registered
 * in the DOM but sent zero POST requests). `supabase.auth.signOut()` is
 * called with NO `scope` argument — default `scope: "global"` — so signing
 * out revokes every refresh token for that staff member everywhere, not
 * just this device.
 */
...
test.describe("desktop", () => {
  test("signing out context A through the sidebar control clears that device's session and revokes context B's session everywhere (global scope)", async ({
    ...
    try {
      ...
      const desktopControl = pageA.getByTestId("retailer-signout-desktop");
      ...
      await pageA.screenshot({
        path: "../../docs/evidence/runs/global-signout-v3/retailer/desktop-signed-in.png",
      });
      ...
      await pageA.screenshot({
        path: "../../docs/evidence/runs/global-signout-v3/retailer/desktop-post-signout-login.png",
      });
      ...
      // session for this staff member, not just context A's.
      ...
      // to /login instead of coasting on stale client state. The retailer
      // app has no guest shell for a protected route, so it redirects.
...
test.describe("mobile", () => {
  test("signing out through the mobile drawer control clears the session", async ({
    ...
    try {
      ...
      const mobileControl = page.getByTestId("retailer-signout-mobile");
      const signOutButton = mobileControl.getByRole("button", {
        name: "Sign out",
      });
      await expect(signOutButton).toBeVisible();
      await page.screenshot({
        path: "../../docs/evidence/runs/global-signout-v3/retailer/mobile-signed-in.png",
      });
      ...
      await page.screenshot({
        path: "../../docs/evidence/runs/global-signout-v3/retailer/mobile-post-signout-login.png",
      });
