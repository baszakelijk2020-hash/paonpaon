...
import { recomputeTemporalHotspot, type AnalyticsActionState } from "./actions";
...
export interface CitedInsightRow {
  readonly id: string;
...
/**
 * The `temporal_hotspot` card. Every row shown is a cited recommendation —
 ...
 */
export function TemporalHotspotCard({
  insights,
}: {
  readonly insights: readonly CitedInsightRow[];
}) {
  const [state, action, pending] = useActionState(
    recomputeTemporalHotspot,
    initial,
  );

  return (
    <div className="flex flex-col gap-3" data-cited-insights="temporal_hotspot">
      {insights.length === 0 ? (
        <p className="text-sm text-[var(--color-stone-500)]">
          No cited finding yet. Recompute once there is appointment history to
          learn from.
        </p>
      ) : (
