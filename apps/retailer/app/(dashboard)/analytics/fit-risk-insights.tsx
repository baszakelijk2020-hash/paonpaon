...
import { recomputeFitRisk, type AnalyticsActionState } from "./actions";
...
/**
 * The `fit_risk` card. Every row shown is a cited recommendation —
 ...
 */
export function FitRiskCard({
  insights,
}: {
  readonly insights: readonly CitedInsightRow[];
}) {
  const [state, action, pending] = useActionState(recomputeFitRisk, initial);

  return (
    <div className="flex flex-col gap-3" data-cited-insights="fit_risk">
      {insights.length === 0 ? (
        <p className="text-sm text-[var(--color-stone-500)]">
          No cited finding yet. Recompute once there is fitting-observation
      ...
      <form action={action} className="flex items-center gap-3">
        <Button type="submit" disabled={pending} id="recompute-fit-risk">
          Recompute
        </Button>
        {state.formError ? (
