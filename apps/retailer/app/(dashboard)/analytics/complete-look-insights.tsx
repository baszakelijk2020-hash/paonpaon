...
import { recomputeCompleteLook, type AnalyticsActionState } from "./actions";
...
/**
 * The `complete_look` card. Every row shown is a cited recommendation —
 ...
 */
export function CompleteLookCard({
  insights,
}: {
  readonly insights: readonly CitedInsightRow[];
}) {
  const [state, action, pending] = useActionState(
    recomputeCompleteLook,
    initial,
  );

  return (
    <div className="flex flex-col gap-3" data-cited-insights="complete_look">
      {insights.length === 0 ? (
        <p className="text-sm text-[var(--color-stone-500)]">
          No cited finding yet. Recompute once there is wardrobe/catalogue
      ...
      <form action={action} className="flex items-center gap-3">
        <Button type="submit" disabled={pending} id="recompute-complete-look">
          Recompute
        </Button>
        {state.formError ? (
