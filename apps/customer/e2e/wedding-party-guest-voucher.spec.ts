...
test("the organizer sees a guest voucher the retailer issued", async ({
  page,
}) => {
  ...
  const party = await partyRepo.create({
    ...
    venueName: "E2E Guest Voucher Venue",
  });
  await partyRepo.issueGuestVoucher({
    retailerId: asId<"RetailerId">(retailer.id),
    weddingPartyId: party.id,
    guestLabel: "E2E Test Guest",
    valueMinorUnits: 12500,
    currency: "EUR",
  ...
  try {
    ...
    const voucherItem = page.locator("li", { hasText: "E2E Test Guest" });
    await expect(voucherItem).toBeVisible();
    await expect(voucherItem).toContainText("€125.00");
    await expect(
      voucherItem.getByText("Active", { exact: true }),
    ).toBeVisible();
  } finally {
