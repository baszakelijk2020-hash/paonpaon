...
> **Status:** Active. **Authority:** Rank 2 — Ratified PAON product
> contract (mechanics/extraction procedure); the per-FT status _table_
> below is a **derived summary**, not an independent source — see "How
> this table relates to `FOUNDER_TOOL_BLUEPRINTS.md`" directly below.
> **Purpose:** where founder-source fragments live, the porting rule, the
> status vocabulary, and a concise per-FT status summary. **Audience:**
> engineers/agents about to touch a founder-designed surface.
> **Canonical scope:** porting mechanics and status summary for FT-01–14
> and the storefront — not the full build contract. **Depends on:**
> `FOUNDER_TOOL_BLUEPRINTS.md` (this table summarizes it; on any
> disagreement, the blueprint's fuller prose wins). **Supersedes:** its
> own pre-2026-08-06 "Audited status — 2026-08-02" table, refreshed for
> drift found in `docs/documentation-audit/DOCUMENT_CONFLICTS.md` #2.
> **Related documents:** `FOUNDER_TOOL_BLUEPRINTS.md`, `../CLAUDE.md`,
> ADR-052/071. **Implementation verification date:** 2026-08-06.
...
## How this table relates to `FOUNDER_TOOL_BLUEPRINTS.md`
...
[FOUNDER_TOOL_BLUEPRINTS.md](./FOUNDER_TOOL_BLUEPRINTS.md) — it is now the
**single canonical source for both the build contract and the current
per-FT status** (see its own 2026-08-06 header note). The table below is a
concise, actively-refreshed _summary_ of that document's status prose, kept
for quick scanning — it is not a second authority. If this table and
`FOUNDER_TOOL_BLUEPRINTS.md` ever disagree, the blueprint's fuller prose is
correct; treat the disagreement as this table having gone stale and refresh
it in the same session, the same way `docs/documentation-audit/DOCUMENT_CONFLICTS.md`
#2 was fixed on 2026-08-06.
...
## Audited status — refreshed 2026-08-06 (originally 2026-08-02)

| Surface                                    | Current implementation                                                                                                                                                                                                                                                                                                                                                                                                   | Honest status and required correction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Voice + drag fit slider                    | `apps/retailer/components/fit-tools/vox-fit-slider.tsx`, `vox-source.ts`, real fitting observations, `fit_profile_candidates` state machine, `fit-tools.spec.ts`                                                                                                                                                                                                                                                         | **Connected first slice (refreshed 2026-08-06).** Reviewed FitProfile candidate/version workflow now has a real caller: advisor proposes from checked observations on the alteration's Fit tools panel, approves/rejects from the customer profile, e2e-proven end to end including a real duplicate-submit assertion (`fit-tools.spec.ts`, 4/4). Three real bugs found wiring the first caller (wrong FK column, write into a table already superseded two weeks earlier, a missing SQL `from` clause) — fixed by migrations `20260806000002`/`20260806000003`. Deliberately not built, named honestly: Web Speech API voice transcription (chip-tap fallback only), supplier write-back, customer-side capture surface, real advisor fit-comparison rendering (data ready, UI deferred), candidate expiry TTL. Full detail: `FOUNDER_TOOL_BLUEPRINTS.md` FT-01.                                                                                                          |
...
| TableService / MunroMessenger              | exact `paon-template.html` widget + typed message API/action/repository                                                                                                                                                                                                                                                                                                                                                  | **Connected first slice (refreshed 2026-08-06).** The exact storefront and React child-route port now send text plus photo/PDF/Pinterest/wedding-fabric material into one private conversation with preview/remove, rights, basic byte/MIME/size/link validation, signed reads and retained failures. Wedding-party linking and garment linking (`message_attachments.wardrobe_item_id`) on attachments are now closed. **New (2026-08-06):** the conversation-to-appointment outcome journey is closed — migration `20260806000000` adds `appointments.origin_message_thread_id` and a SECURITY DEFINER `book_appointment_from_consultation()` RPC, e2e-proven for customer, retailer staff and unauthorized-rejection callers (`consultation-outcome.spec.ts`). Still incomplete: async malware/quarantine, attachment progress UI, proposal/cart creation path, consent-withdrawal/retention proof, AI citation proof. Full detail: `FOUNDER_TOOL_BLUEPRINTS.md` FT-09. |
| Inspiration Box / gift booklet             | none found (reconfirmed 2026-08-02 — the only source occurrence is one static marketing sentence, not a component)                                                                                                                                                                                                                                                                                                       | **Connected first slice (refreshed 2026-08-06)**, built with PAON primitives since there is no source fragment to port: `apps/retailer/app/(dashboard)/gifts` (curate/invite) and `apps/customer/app/r/[slug]/gift/[token]` (anonymous reveal/redeem). Resend/initial-send closed (2026-08-02, `enqueue_gift_invitation_email` RPC). Expiry/revoke was found already fully wired but never exercised end to end — now proven via `gift-revoke.spec.ts` (2026-08-06), no defect found. Still incomplete: giver payment/request flow, recall/refund handling. Full detail: `FOUNDER_TOOL_BLUEPRINTS.md` FT-10.                                                                                                                                                                                                                                                                                                                                                               |
...
| Pag2 groom/best-men planner                | party/member/invite/fitting-state model, generic pages, faithful orbit, additional domain/schema                                                                                                                                                                                                                                                                                                                         | **Functional foundation, fully wired across existing schema (refreshed 2026-08-06).** Aftercare plans, group fittings, inspiration items, design choices, group-date agreement (propose/vote/finalize), and guest vouchers are all connected via SECURITY DEFINER RPCs with organizer-or-member authorization re-derived server-side. **A real cross-tenant integrity bug was found and fixed 2026-08-06**: five wedding-party child tables' staff-insert RLS checked only `retailer_id`, never that `wedding_party_id` belonged to the same retailer — fixed via a composite FK; proven by new pgTAP coverage. Planner workflow/experience (the faithful visual/sequencing port of `pag2.html`) otherwise remains partial. Full detail: `FOUNDER_TOOL_BLUEPRINTS.md` FT-13.                                                                                                                                                                                               |
| Pag3 Preferred Tailoring / HighMaintenance | service plans, entitlements, bookings, care/cost/history and partner schema; generic pages                                                                                                                                                                                                                                                                                                                               | **First connected weekly-plan customer slice (refreshed 2026-08-06, lane-e).** Retailer-side `/services` primitives (plan creation, activation, enrollment, credit grants, booking requests, care/cost records) proven via `apps/retailer/e2e/services.spec.ts`. An advisor now proposes a versioned weekly wardrobe plan (day/occasion/notes) per membership; the customer sees and accepts/declines it via `decide_service_weekly_plan` (re-derives the caller server-side, same pattern as `complete_wedding_aftercare_plan`), proven by `apps/customer/e2e/weekly-plan.spec.ts` and 12 pgTAP assertions. `pag3.html`'s Preferred Tailoring section has no discrete interactive fragment (confirmed by direct inspection), so this is curated from the blueprint's own description, not a pixel port. Custody handoffs, the partner portal and cost reconciliation remain unbuilt. Full detail: `FOUNDER_TOOL_BLUEPRINTS.md` FT-14.                                     |
