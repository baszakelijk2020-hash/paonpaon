...
- **Immediate facts:** `ENVIRONMENTS.md` is the project ledger. Local Supabase
  ...
  workshop, service-plan, order, campaign, loyalty, event, collection,
  ...
  pixel port — CSS, markup, image URLs and the model configs are
  byte-for-byte from source. GSAP scroll/opacity tweening is reimplemented
  with a hand-rolled requestAnimationFrame tween (GSAP's own power2.inOut
  formula) and a CSS transition, since this codebase has no GSAP dependency
  (same precedent as `am-house-orbit.tsx`). Migration `20260802000008` adds
  `suit_configuration_intents` (append-only) and
  `save_suit_configuration_intent`, a narrow RPC re-deriving the caller's
  Customer row and self-creating it on first interaction (same shape as
  `save_wishlist_item`). Customers explore at `/r/[slug]/configurator`
  (gated by `wardrobe_styling`) and make an explicit save decision. Proof: a
  browser journey covering initial predefined-model state, model-click
  resync of all three sub-carousels, save, and a database assertion.
...
spec.ts` proved exactly one of five "Needs your attention" card types
  ...
  create a task. Instead, a new `add_alteration_task` RPC (advisor-only)
...
.findRecentByCustomer` already existed with no caller, and its
  ...
  green.
  Virtual Wardrobe Studio 4.9 and 4.10 are reconciled on Lane H: advisors
  compose and enqueue roadmap-linked Outfits, customers review each ready
  look, batch-enqueue/cancel their saved looks and feed consented garment-taste
  feedback into the existing StyleProfile evidence path. The same-House
  advisor cancellation boundary, persisted-consent browser fixtures and the
...
| CAMP-001               | Guided seven-day catalogue wardrobe/look creation with elegant tie/shirt/short-lived rewards — **parked as a standalone programme (founder decision, 2026-08-12); Honeymoon Phase (PHASE.md 10.2) is the active order-to-wardrobe replacement** | Campaigns                          | Challenge enrollments, seven catalogue looks, idempotent completion, capped rewards                                                         | CUST-002, ROAD-002                              | 5.1                  | Authenticated customer composes seven complete looks; deterministic completion and restrained reward are auditable               | Complete historically; parked for active development                                                      |
...
offer — **parked as a standalone programme (founder decision, 2026-08-12)**;
the active order-to-wardrobe programme is **Honeymoon Phase**
(`PHASE.md` 10.2), a very-high-priority journey across anticipation,
preparation, delivery, first wear, care and advisor follow-through, not a
seven-day challenge. Audiences are consent-aware and explainable; delivery and
suppression are auditable. PAON does not use mass-retail discount
gamification.
