...
- **Completed programme commits:** `dd695d5` authorized the Intelligence
  ...
  metadata persistence, repositories, and RLS; `HEAD` adds the review
  transition contract and PAON Admin canonical management.
- **Available schema/interfaces:** seven metadata/fabric tables, generated
  ...
  `review_metadata_assignment`, atomic `set_product_fabric_profile`, and
  operator-only canonical concept forms now implement the landed contracts.
- **Checks/deployment state:** a clean local database reset applies all 91
  migrations; 39 live pgTAP authorization/invariant checks, 139
  ...
  Supabase advisor findings. The current slice passes focused lint, strict
  typecheck, unit tests, and the repository-wide gate recorded in its commit.
- **Real blockers:** none for Stage 1. Missing optional provider credentials do
  not block local/domain/database work.
- **Exact next files/tests:** finish queue item 1.3 with tenant proposal,
  assignment review, and local override Server Actions/views under
  `apps/retailer/app/(dashboard)/metadata/`, plus its dashboard navigation.
  Reuse the landed review schemas and `MetadataRepository`; cover manager
  authorization and accessible pending/error/empty states. Do not reopen the
  review transition, audit, or canonical-management design unless code
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| CAT-001                | Universal catalogue graph for mills, collections, fibres, weaves, patterns, colours, seasons, garment types, construction, fit, formality, climate, performance, care, style, occasions, and compatibility | `metadata`                         | Typed concepts, edges, assignments, persistence, RLS, repositories, and canonical management exist; tenant review UI is next | Direction                                 | 1.1–1.3              | Typed concepts/edges exist; accepted assignments cover every named kind; unknowns remain reviewable proposals                    | In progress (1.3)  |
...
| CAT-003                | Retailer overrides, canonical ownership, and strict tenant isolation                                                                                                                                       | `metadata`                         | Canonical management and tenant-safe domain/database/repository boundaries exist; tenant review UI remains                   | CAT-001                                   | 1.2–1.4              | Canonical rows are platform-owned; retailer rows/overrides cannot cross tenants or mutate canonical facts                        | In progress (1.3)  |
| CAT-004                | Provenance, source, confidence, evidence, review state, and auditable review                                                                                                                               | `metadata`                         | Pending proposals and actor-derived idempotent terminal reviews retain append-only evidence; tenant UI remains               | CAT-001                                   | 1.1–1.3              | Every proposal records authority and evidence; AI never bypasses pending review; review actor/time are retained                  | In progress (1.3)  |
