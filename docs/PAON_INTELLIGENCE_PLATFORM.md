...
- **Current queue item:** `1.3 Metadata review workflow`
- **Current requirement IDs:** `CAT-003`, `CAT-004`, `ENG-001`
- **Completed programme commits:** `dd695d5` authorized the Intelligence
  ...
  `e05ac65` implemented metadata domain contracts; `HEAD` implements metadata
  persistence, repositories, and RLS.
- **Available schema/interfaces:** seven metadata/fabric tables, generated
  ...
  append-only terminal-review evidence, and the atomic
  `set_product_fabric_profile` RPC now implement the `@paon/domain` contracts.
- **Checks/deployment state:** a clean local database reset applies all 90
  migrations; 22 live pgTAP authorization/invariant checks and 124
  `@paon/database` tests pass; database lint reports only the pre-existing
  `convert_pilot_to_live_retailer` warning; metadata/fabric tables produce no
  Supabase advisor findings; frozen install, lint, strict typecheck, unit
  tests, all three production builds, and format check pass.
- **Real blockers:** none for Stage 1. Missing optional provider credentials do
  not block local/domain/database work.
- **Exact next files/tests:** extend the metadata review repository methods,
  then add focused canonical management under
  `apps/admin/app/(dashboard)/metadata/` and tenant review under
  `apps/retailer/app/(dashboard)/metadata/`, following the existing session,
  Server Action, form-state, and navigation patterns. Cover authorization,
  transition/idempotency, audit retention, cross-tenant denial, and accessible
...
Verified from code and 90 migrations on 2026-07-30:
...
- Metadata concepts/edges/assignments, append-only review evidence, retailer
  overrides, exact fabric profiles, generated types, and typed repositories
  now exist. The repository still has no metadata management/review UI,
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| CAT-001                | Universal catalogue graph for mills, collections, fibres, weaves, patterns, colours, seasons, garment types, construction, fit, formality, climate, performance, care, style, occasions, and compatibility | `metadata`                         | Typed concepts, edges, assignments, persistence, RLS, and repositories exist; review UI is next     | Direction                                 | 1.1–1.3              | Typed concepts/edges exist; accepted assignments cover every named kind; unknowns remain reviewable proposals                    | In progress (1.2)  |
| CAT-002                | Exact composition percentages, fabric weight, supplier reference, and variant-specific facts where genuinely different                                                                                     | `metadata` + `catalog`             | Exact validated facts persist atomically; product management UI is not yet connected                | CAT-001                                   | 1.1, 1.4             | Composition validates to 100%; exact values persist without duplicating concept labels                                           | In progress (1.2)  |
| CAT-003                | Retailer overrides, canonical ownership, and strict tenant isolation                                                                                                                                       | `metadata`                         | Domain rules, database triggers, RLS, grants, repositories, and live tenant tests enforce ownership | CAT-001                                   | 1.2–1.4              | Canonical rows are platform-owned; retailer rows/overrides cannot cross tenants or mutate canonical facts                        | In progress (1.2)  |
| CAT-004                | Provenance, source, confidence, evidence, review state, and auditable review                                                                                                                               | `metadata`                         | Assignments persist provenance and terminal review snapshots; management/review workflow is next    | CAT-001                                   | 1.1–1.3              | Every proposal records authority and evidence; AI never bypasses pending review; review actor/time are retained                  | In progress (1.2)  |
