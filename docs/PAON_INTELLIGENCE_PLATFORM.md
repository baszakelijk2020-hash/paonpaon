...
- **Current queue item:** `1.4 Exact product facts and catalogue assignment UI`
- **Current requirement IDs:** `CAT-001`, `CAT-002`, `CAT-004`, `ENG-003`
- **Completed programme commits:** `dd695d5` authorized the Intelligence
  ...
  the Retailer Portal metadata review UI (PHASE 1.3 complete).
- **Available schema/interfaces:** seven metadata/fabric tables, generated
  ...
  operator-only canonical concept forms, and Retailer Portal
  propose/review/override Server Actions under
  `apps/retailer/app/(dashboard)/metadata/` now implement the landed
  contracts.
- **Checks/deployment state:** a clean local database reset applies all 91
  migrations; 39 live pgTAP authorization/invariant checks, 145
  ...
  integrated 1.3 main tip.
- **Real blockers:** none for Stage 1. Missing optional provider credentials do
  not block local/domain/database work.
- **Exact next files/tests:** implement queue item 1.4 on Retailer Portal
  product management (`apps/retailer/app/(dashboard)/products/`) so staff can
  manage product/variant metadata assignments plus exact fabric weight,
  supplier reference, and concept-linked composition. Reuse
  `ProductFabricProfileRepository` and `MetadataRepository`; keep variant
  fabric facts only for real variant differences; never copy accepted labels
  into parallel string fields. Cover composition validation/rollback,
  product-versus-variant precedence, authorization, and accessible form
  states. Do not reopen the metadata review workflow or add import/search UI.
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| CAT-001                | Universal catalogue graph for mills, collections, fibres, weaves, patterns, colours, seasons, garment types, construction, fit, formality, climate, performance, care, style, occasions, and compatibility | `metadata`                         | Typed concepts, edges, assignments, persistence, RLS, repositories, Admin + Retailer review UI exist; product assignment UI is next | Direction                                 | 1.1–1.4              | Typed concepts/edges exist; accepted assignments cover every named kind; unknowns remain reviewable proposals                    | In progress (1.4)  |
| CAT-002                | Exact composition percentages, fabric weight, supplier reference, and variant-specific facts where genuinely different                                                                                     | `metadata` + `catalog`             | Exact validated facts persist atomically; product management UI is not yet connected                                                | CAT-001                                   | 1.1, 1.4             | Composition validates to 100%; exact values persist without duplicating concept labels                                           | In progress (1.4)  |
