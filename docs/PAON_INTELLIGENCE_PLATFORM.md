...
- **Completed programme commits:** `dd695d5` authorized the Intelligence
  ...
  MorningRoutine selection and actions (PHASE 4.4 complete).
- **Available schema/interfaces:** seven metadata/fabric tables, four
  ...
  grounded-answer/morning-routine/wardrobe/sartorial/outfit/roadmap/lifecycle
  contracts, `@paon/ai` `generateGroundedAnswer`, customer account consent +
  StyleProfile + wardrobe roadmap/lifecycle/self-scan/fit-freshness/
  morning-routine controls, consented storefront/swipe/TableService producers,
  Retailer Portal advisor brief/wardrobe/roadmap/lifecycle mounts, Customer
  `/morning-routine` in-app view, and TableService occasion guidance with
  swipe/appointment conversion hooks.
- **Checks/deployment state:** 104 migrations; MorningRoutine selection/
  actions domain/repository/surface and lint/typecheck/test/build/format are
  green on the 4.4 tip. Anonymous interaction persistence remains blocked
  pending jurisdiction documentation.
- **Real blockers:** none for Stage 4.5 MorningRoutine delivery beyond
  ordinary missing Resend key for live email smoke only; anonymous
...
- **Exact next files/tests:** implement queue item 4.5 MorningRoutine
  delivery and retailer controls: in-app notification/email outbox
  orchestration, subscription/frequency/quiet-period controls, delivery audit,
  suppression, and retailer eligible-product controls.
...
| MR-001                 | Opt-in location, weather, temperature, calendar/occasion, wardrobe, catalogue, and StyleProfile inputs                                                                                                     | MorningRoutine                     | MorningRoutine provider interfaces, provenance, and repository projection exist                                                                      | ROAD-001, CUST-003                        | 4.4                  | Each input has provenance/consent/fallback; location is optional and separately revocable                                        | Done (4.4)                                                                                                |
| MR-002                 | Daily in-app/email recommendations with one-tap save, review, appointment, or purchase paths                                                                                                               | MorningRoutine                     | In-app selector and action paths exist; email delivery remains Stage 4.5                                                                             | MR-001                                    | 4.4–4.5              | Owned garments rank first; recommendations explain why; delivery is opt-in, scheduled, auditable, and unsubscribeable            | Partial (4.4 in-app)                                                                                      |
...
| ENG-002                | Recommendations and AI answers use accepted metadata/approved knowledge, explain why, and respect consent/retention                                                                                        | Intelligence governance            | Discovery ranking, catalogue intent, TableService grounded answers, and MorningRoutine selection cite approved objects with consent-aware provenance | EDU-002, CUST-001                         | 2.2–4.5              | Every output exposes evidence/explanation and fails closed when approval or consent is absent                                    | Partial (4.4)                                                                                             |
